package com.example.streamline.adapter
...
import com.example.streamline.databinding.BookmarkViewBinding
import com.example.streamline.databinding.LongBookmarkViewBinding
        ...
        holder.root.setOnClickListener{
            when {
                checkForInternet(context) -> {
                    changedTab(MainActivity.bookmarkList[position].name,
                BrowseFragment(urlNew = MainActivity.bookmarkList[position].url))
                    if(isActivity) (context as Activity).finish()
                }
                else -> Snackbar.make(holder.root, "Internet is Not Connected\uD83D\uDE03", 3000).show()
            }
